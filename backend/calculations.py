...
def calculate_mock_analytics(strikes_data, spot, futures,
                             expiry_datetime, contract_multiplier=50):
    """
    Lightweight analytics for MOCK mode.
    - Skips implied_volatility() → no brentq
    - Skips true_gamma_flip_vectorized() → no brentq
    - Uses synthetic IV + closed-form BS Greeks
    """
    ...
    for strike in strikes_data:
        for opt_type in ["CE", "PE"]:
            ...
            # ── Synthetic IV (no brentq) ──────────────────────────
            # ATM = ~18%, wings widen realistically
            distance = abs(strike - spot) / spot
            synthetic_iv = 0.18 + distance * 0.6 + random.uniform(-0.015, 0.015)
            synthetic_iv = max(0.08, min(0.65, synthetic_iv))

            greeks = calculate_greeks(spot, strike, T, RISK_FREE_RATE, synthetic_iv, opt_type)
            gex = 0.0
            if greeks:
                gex = calculate_gex(greeks["gamma"], oi, opt_type, contract_multiplier)
                net_gex += gex
    ...
    # ── Mock gamma flip: simple sign-scan (no brentq) ─────────
    gamma_flip = None
    all_strikes = sorted(strikes_data.keys())
    cumulative = 0.0
    for strike in all_strikes:
        ce_gex = strikes_data[strike].get("CE", {}).get("gex", 0)
        pe_gex = strikes_data[strike].get("PE", {}).get("gex", 0)
        cumulative += ce_gex + pe_gex
        if cumulative > 0:
            gamma_flip = strike
            break

    futures_spread = None
    if futures is not None and spot > 0:
        futures_spread = futures - spot
